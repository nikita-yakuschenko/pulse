...
/**
 * Элемент вложения из ответа 1С (поле Вложения при full=true).
 * 1С возвращает: { ИмяФайла, Данные (base64) }
 * Старые поля оставлены для обратной совместимости.
 */
export interface SupplierOrderAttachment {
  /** Имя файла */
  ИмяФайла?: string
  /** Содержимое файла в Base64 (поле Данные в ответе 1С при full=true) */
  Данные?: string
  /** Устаревшее: содержимое в Base64 */
  ФайлBase64?: string
  /** Устаревшее: UUID записи хранилища */
  ИдентификаторХранилища?: string
  /** Устаревшее: есть ли двоичное вложение */
...
export interface SupplierOrder {
  ...
  /** Сумма заказа (1С может возвращать СуммаДокумента) */
  Сумма?: number
  СуммаДокумента?: number
  /** Вложения (файлы) из ответа 1С при full=true */
  Вложения?: SupplierOrderAttachment[]
  /** Устаревшее: старый формат вложений */
...
export interface Payment {
  /** Дата документа в формате "DD.MM.YYYY HH:MM:SS" */
  Дата: string
  /** Дата расхода (планируемая/фактическая дата списания) — приоритетна для отображения в таблице заявок на оплату */
  ДатаРасхода?: string
  /** Номер документа (АС000000001, ИПК00000001) */
  Номер: string
  /** Сумма оплаты (1С может возвращать СуммаДокумента) */
  Сумма?: number
  СуммаДокумента?: number
  ...
  Контрагент: string
  /** Получатель (в 1С может приходить вместо/вместе с Контрагент; для отображения использовать получатель при пустом контрагенте) */
  Получатель?: string
  /** Наша организация */
  Организация: string
...
export interface Receipt {
  ...
  /** Организация */
  Организация?: string
  /** Сумма (1С может возвращать СуммаДокумента) */
  Сумма?: number
  СуммаДокумента?: number
  [key: string]: unknown
}
